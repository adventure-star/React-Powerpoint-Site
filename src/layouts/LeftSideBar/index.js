...
const LeftSideBar = (props) => {
    ...
    const SideBarLinks = links.map((link) => (
        <Link to={link.link} key={link.name} className="relative flex items-center py-6 hover:bg-blue-700">
            <div className="flex-none xl:flex items-center pl-0 xl:pl-8 mx-0 md:mx-auto xl:mx-0">
                <img src="/images/navpicker.png" className={`${pathName == link.link ? `block` : `hidden`} absolute left-0`} />
                <div className="mx-0 md:mx-auto xl:mx-0" style={{ width: "35px" }}>
                    <img src={link.src} className="mx-auto xl:mx-0" />
                </div>
                <p className="text-white pl-6 md:pl-0 xl:pl-3 pt-0 md:pt-2 xl:pt-0 font-custom-medium text-xl whitespace-pre">{link.name}</p>
            </div>
        </Link>
    ));
    ...
    return (
        <div className="w-full flex bg-customPrimary relative" style={{ minHeight: "100vh" }}>
            ...
            <div className="flex-none md:flex w-full" style={{ zIndex: "10" }}>
                <div className="w-full md:w-1/5 2xl:w-full 2xl:max-w-310 pt-3 pb-3 md:pt-20 md:pb-0 md:h-auto" style={{ minHeight: "64px" }}>
                ...
                <div className="w-full md:w-4/5 2xl:w-full bg-white rounded-none md:rounded-tl-large" style={{ zIndex: "2" }}>
                    {children}
                </div>
            </div>
        </div>
